...
static int loop_clr_fd(struct loop_device *lo)
{
...
#ifdef CONFIG_RSBAC
	rsbac_pr_debug(aef, "[lo_ioctl()]: calling ADF for FILE/DEV\n");
	if (S_ISREG(lo->lo_backing_file->f_path.dentry->d_inode->i_mode)) {
		rsbac_target = T_FILE;
		rsbac_target_id.dir.device = lo->lo_backing_file->f_path.dentry->d_sb->s_dev;
		rsbac_target_id.dir.inode  = lo->lo_backing_file->f_path.dentry->d_inode->i_ino;
		rsbac_target_id.dir.dentry_p = lo->lo_backing_file->f_path.dentry;
	}
	else { /* must be block dev */
		rsbac_target = T_DEV;
		rsbac_target_id.dev.type = D_block;
		rsbac_target_id.dev.major = RSBAC_MAJOR(lo->lo_backing_file->f_path.dentry->d_inode->i_rdev);
		rsbac_target_id.dev.minor = RSBAC_MINOR(lo->lo_backing_file->f_path.dentry->d_inode->i_rdev);
	}
	rsbac_attribute_value.dummy = 0;
	...
	rsbac_pr_debug(aef, "loop_clr_fd() [lo_ioctl()]: calling ADF for DEV\n");
	rsbac_target_id.dev.type = D_block;
